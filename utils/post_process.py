# Based on ideas from: https://gitlab.com/UnBVision/edgenet360/-/blob/master/lib_edgenet360/post_process.py?ref_type=heads
...
def fill_limits_vox(vox, max_x,min_x, max_z,min_z):
    ...
    ####################################################################################################
    # check the scene coverage space 
    if not ((max_x + min_x)> 60 or (max_z + min_z)>60):
    
        #clear the voxels outside the specified X boundaries
        vox[:min_x, :, :] = 255 
        vox[max_x+1:, :, :] = 255 
        # clear the voxels outside the specified Z boundaries
        vox[:, :, :min_z] = 255  
        vox[:, :, max_z+1:] = 255
        
        
        # Set the walls before the boundaries directly
        vox[max(xs-1, 0), 1:ye + 1, zs:ze + 1] = 3
        vox[min(xe, vox.shape[0]-1), 1:ye + 1, zs:ze + 1] = 3
        vox[xs:xe + 1, 1:ye, max(zs-1, 0)] = 3
        vox[xs:xe + 1, 1:ye, min(ze, vox.shape[2]-1)] = 3
        ...
        # Set the floor
        vox[min_x:max_x + 1, 0, min_z:max_z + 1] = 2
        
        # Set the ceiling
        vox[min_x:max_x + 1, ye-7, min_z:max_z + 1] = 1
        
        
    else:
        # Set the walls on the boundaries
        vox[max(xs-1, 0), 1:ye + 1, zs:ze + 1] = 3
        vox[min(xe, vox.shape[0]-1), 1:ye + 1, zs:ze + 1] = 3
        vox[xs:xe + 1, 1:ye, max(zs-1, 0)] = 3
        vox[xs:xe + 1, 1:ye, min(ze, vox.shape[2]-1)] = 3
        ...
        # Set the floor
        vox[xs:xe + 1, 0, zs:ze + 1] = 2
         
        # Set the ceiling
        vox[xs:xe + 1, ye-7, zs:ze + 1] = 1
...
def fill_limits_vox_stanford(vox):
    xs, ys, zs = 0, 0, 0
    xe, ye, ze = np.array(vox.shape) - 1
...
def clear_out_of_boundaries(vox, max_x,min_x, max_z,min_z):
